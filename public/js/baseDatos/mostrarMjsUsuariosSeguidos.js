...
const getMsjUsuariosSeguidos = async (inicio) => {
    let formData = new FormData();
    formData.append('username', nombreUsuario);
    formData.append('inicio', inicio);

    const resMensajes = await fetch('Model/mensajesUsuariosSeguidos.php', {
        method: 'POST',
        body: formData,
    });

    const datos = await resMensajes.json(); //devuelve un arreglo [mensajes, cantMensajes]
...
const actualizarMsjUsuarios = async (inicio) => {
    ...
    datos = await getMsjUsuariosSeguidos(inicio);
    ...
    if (totalPaginas > 1) {
        ...
        let paginacion = crearVistaPaginacion(inicio, totalPaginas, 'actualizarMsjUsuarios');
...
const crearVistaPaginacion = (pagina, totalPaginas, funcionOnClick) => {
    ...
    if (pagina == 1 || pagina == totalPaginas) {
        //condicion para saber si estoy en la primer pag o el la ult
        divOpcion = crearElemento('div', ['opcion'], '');
        a = crearElemento('a', [''], '');
        a.setAttribute('href', 'javascript:void(0)');

        if (pagina == 1) {
            //si estoy en la primer pag no creo el link a la anterior
            a.setAttribute('onclick', `${funcionOnClick}("${pagina + 1}");`);
            a.textContent = 'Siguiente';
        } else {
            //si estoy en la ult pag no creo el link a la siguiente
            a.setAttribute('onclick', `${funcionOnClick}("${pagina - 1}");`);
...
const crearMensaje = async (mensaje) => {
    ...
    imgFotoPerfil.setAttribute(
        'src',
        `data:image/${datosUsuarioMsj.foto_tipo};base64, ${datosUsuarioMsj.foto_contenido}`
    );
    ...
    if (mensaje.texto != '') {
        //se crea texto del msj en caso de que exista
    ...
    if (mensaje.imagen_tipo != null) {
        //se crea el contenedor para la img en caso de que esta exista en el msj
        divContenendorImagenSubida = crearElemento('div', ['contenedor-file'], '');
        img = crearElemento('img', [], '');
        img.setAttribute(
            'src',
            `data:image/${mensaje.imagen_tipo};base64, ${mensaje.imagen_contenido}`
        );
    ...
    if (dioMeGusta != -1) {
        //comprueba si dio me gusta el usuario logeado al msj
