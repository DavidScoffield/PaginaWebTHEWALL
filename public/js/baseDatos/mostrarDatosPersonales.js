...
const mostrarDatos = async (datos) => {
    ...
    const contenedorImg = Array.from(
        contenedorPerfil.getElementsByClassName('contenedor-img')
    )[0];
    contenedorImg.firstElementChild.setAttribute(
        'src',
        `data:image/${datos.foto_tipo};base64, ${datos.foto_contenido}`
    );
    const contenedorNombre = Array.from(
        contenedorPerfil.getElementsByClassName('contenedor-nombre')
    )[0];
    contenedorNombre.firstElementChild.textContent = datos.nombre;
    const contenedorApellido = Array.from(
        contenedorPerfil.getElementsByClassName('contenedor-apellido')
    )[0];
    contenedorApellido.firstElementChild.textContent = datos.apellido;
    const contenedorNombreUsuario = Array.from(
        contenedorPerfil.getElementsByClassName('contenedor-username')
    )[0];
    contenedorNombreUsuario.firstElementChild.textContent =
        '@' + datos.nombreusuario;

    //ACCIONES A HACER EN EL CASO DE ESTAR EN EL PERFIL DE UN USUARIO QUE NO ES EL MIO
    if (document.getElementById('userperfil')) {
        const idFollow = await consultarSeguimiento(
            nombreUsuarioLogeado,
            nombreUsuario
        );
        const contenedorBtnFollowPerfil = document.getElementById(
            'btnFollowPerfil'
        );
        ...
        if (idFollow != -1) {
            ...
            contenedorBtnFollowPerfil.firstElementChild.textContent =
                'No seguir';
        } else {
