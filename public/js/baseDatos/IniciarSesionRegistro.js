...
const errorLogin = (form) => {
    contenedoresImputs = Array.from(
        form.getElementsByClassName('contenedor-input')
    );
    for (const contenedorInput of contenedoresImputs) {
        contenedorInput.classList.add('error');
    }
    insertarMsjError(
        contenedoresImputs[0],
...
const errorRegister = (form, FormData) => {
    contenedorNombreUsuario = document.getElementById('register_user')
